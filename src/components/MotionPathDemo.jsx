...
// animejs をクライアント側でのみ動的 import
export default function MotionPathDemo({ duration = 5000, rotate = true }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // パッケージルートを動的 import (exports フィールド対応)
      const mod = await import("animejs").catch((e) => {
        console.error("animejs load failed", e);
        return null;
      });
  ...
  return (
    ...
    >
      <svg viewBox="0 0 304 112" width={304} height={112}>
        <title>Suzuka</title>
